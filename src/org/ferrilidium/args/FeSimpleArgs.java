...
public class FeSimpleArgs {
	...
	public String[] tokenize2 (String args) {
		List<String> accum = new ArrayList<>();
		Matcher m = Pattern.compile(regex2).matcher(args);
	    while (m.find()) {
	        if (m.group(1) != null) {
	        	accum.add(m.group(1));
	        } else {
	...
	ParseState getNextToken(String[] tokens, int idxFirstValue) {
		...
		/** Returns true if token in the stringbuilder is "complete", which either means we have name=value or just name and there's no =value.
		 * Note that value can be a quoted string, and that it's possible for the start and ending quote to be in different tokens.
		 */
	boolean tokenComplete (StringBuilder sb, String[] tokens, int idx) {
